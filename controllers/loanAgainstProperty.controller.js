...
const normalizeTransactionBankNames = (selectedBanks, otherBankName) => {
  ...
  // Accepts string | array | { displayName, banks } — mirrors the business-loan controller.
  if (Array.isArray(selectedBanks)) {
    pushUnique(selectedBanks);
  } else if (selectedBanks && typeof selectedBanks === "object") {
    pushUnique(selectedBanks.banks);
  } else {
    pushUnique(selectedBanks);
  }
...
const sanitizeEmploymentFields = (data) => {
  ...
  if (employmentType === "Salaried") {
    ...
    PROFESSIONAL_ONLY_FIELDS.forEach((field) => allowed.delete(field));
